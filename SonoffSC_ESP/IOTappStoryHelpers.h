...
void loopWiFiManager() {
  ...
  // additional fields
  WiFiManagerParameter p_updateInterval("updateInterval", "updateInterval", config.updateInterval, STRUCT_CHAR_ARRAY_SIZE);
  ...
  wifiManager.addParameter(&p_updateInterval);
  ...
  //add all parameters here
  strcpy(config.updateInterval, p_updateInterval.getValue());
     ...
     //   DEBUG_PRINTLN(buf.get());
        ...
        if (json.success()) {
          ...
          if (json.containsKey("magicBytes")) {
            ...
            strcpy(config.updateInterval, json["updateInterval"]);
            strcpy(config.mqttServer, json["MQTT_SERVER"]);
            strcpy(config.mqttPort, json["MQTT_PORT"]);
            strcpy(config.mqttUser, json["MQTT_USER"]);
            strcpy(config.mqttKey, json["MQTT_KEY"]);
            strcpy(config.mqttTopic, json["MQTT_TOPIC"]);
