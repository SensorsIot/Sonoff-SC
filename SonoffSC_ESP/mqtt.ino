...
/*void buildTopics() {
  mqttTopic = MQTT_USER  + MQTT_TOPIC;
...
void mqttSend(const char * topic, int message) {
  ...
  String path = String(config.mqttUser) + String(config.mqttTopic) + String(topic);
  Serial.println(path);
...
//  buildTopics();
 ...
 // mqttSend(MQTT_IP_TOPIC, WiFi.localIP());
...
void mqttConnect() {
  ...
  if (!mqtt.connected()) {
    ...
    char * host = strdup(config.mqttServer);
    if (strlen(host) == 0) return;
    unsigned int port = atoi(config.mqttPort);
    char * user = strdup(config.mqttUser);
    char * pass = strdup(config.mqttKey);
